...
function LockScreen({ logged }) {
    ...
    const handleKeyDown = useCallback((e) => {
        if (lockScreenProps.isLocked && e.keyCode == 32) {
        ...
        <main className='main'>
            ...
            <div className={unlockScreenProps.className}>
                <div className="container">
                    ...
                    {/* If the form is not submitted, show the form, otherwise show the loading icon*/}
                    {
                        !unlockScreenProps.submited &&
                        <form className="input" onSubmit={submitForm}>
                            <input type={unlockScreenProps.inputType} id='login' placeholder="Password" autoComplete='off' />

                            <button type='button' onClick={changeInputType} className='change-input-type-btn'>
                                {
                                    unlockScreenProps.inputType === 'password' ? <FaRegEyeSlash className='icon' size={15} /> : <FaRegEye className='icon' size={15} />
                                }
                            </button>
                    ...
                    {
                        unlockScreenProps.submited && <AiOutlineLoading className='loading' size={50} />
                    }
                </div>
            </div>
        </main>
